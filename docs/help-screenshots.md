...
撮影時には、環境で画面が変わってしまうものを2つ固定しています。
check のためだけでなく、**撮る場所で画像が変わらないように**するためでもあります。

| 固定するもの | 固定しないと |
|---|---|
| タイムゾーン（`Asia/Tokyo`） | デモの時刻は固定の瞬間なので、撮る人の場所で表示時刻が変わる |
| 読み上げの音声の有無 | 音声が入っていない端末では読み上げボタン（▶）が消える |
...
- `/#/dev/seed` は開発サーバーでだけ開けます（本番ビルドには入りません）。
  手元のブラウザで開くと、そのブラウザの探究とノートがデモのデータで置き換わり、画面の言語と学ぶ言語も切り替わるので注意してください（保存済みの API キーはそのまま残ります）。
